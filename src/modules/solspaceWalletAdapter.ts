...
interface SolspaceWindow extends Window {
  Solspace?: {
    new (): SolspaceWallet;
  };
  slopeApp?: unknown;
...
declare const window: SolspaceWindow;

export interface SolspaceWalletAdapterConfig {}

export const SolspaceWalletName = 'Solspace Wallet' as WalletName;

export class SolspaceWalletAdapter extends BaseMessageSignerWalletAdapter {
  name = SolspaceWalletName;
  ...
  private _wallet: SolspaceWallet | null;
  ...
  constructor(config: SolspaceWalletAdapterConfig = {}) {
    super();
    ...
    if (this._readyState !== WalletReadyState.Unsupported) {
      scopePollingDetectionStrategy(() => {
        if (typeof window.Solspace === 'object') {
  ...
  async connect(): Promise<void> {
    try {
      ...
      const wallet = new window.Solspace();
      ...
      try {
        ...
        if (parsed && parsed.message === 'connect') {
          data = parsed.data;
        }
        console.log(data);
      } catch (error: any) {
